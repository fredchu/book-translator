...
## Known limitations / before going public

This repo is **private** and currently has one blocker for public release:

- **Co-Intelligence translation strings are hardcoded in `scripts/regenerate_bilingual.py` and `scripts/assemble.py`.** During the structure-preservation refactor (Round 2 + Round 3 of the autonomous fix loop), the assembler was given hardcoded English→繁中 paragraph pairs for the Co-Intelligence acknowledgments page, copyright page, and dedication, as a fast path to get the regeneration audit passing. Those strings are copyrighted text from Ethan Mollick's *Co-Intelligence: Living and Working with AI* (Portfolio/Penguin, 2024). They must be lifted out of the skill source and into a book-specific translation table (kept outside the repo, under the run directory) before the repo can go public.
- The `nav.xhtml` template in `assemble.py` also has Co-Intelligence-specific chapter labels hardcoded. It needs to become a generic template that reads chapter titles from the input manifest.
- `scripts/regenerate_bilingual.py` has three hardcoded `/Users/fredchu/...` paths that need to become required CLI arguments.

None of these affect the skill's design — they're artifacts of Codex taking the cheapest path to satisfy the audit during automated refactoring. The fix is a focused refactor of those three scripts; the rest of the pipeline is already book-agnostic.

Once those are out, public-release checklist:

- [ ] Lift Co-Intelligence-specific strings into an external translation table
- [ ] Make `regenerate_bilingual.py` paths CLI args
- [ ] Add `LICENSE` (MIT or Apache 2.0)
- [ ] Add `CHANGELOG.md`
- [ ] Add CI (GitHub Actions pytest)
- [ ] Set GitHub topics: `claude-code`, `epub`, `translation`, `bilingual`, `traditional-chinese`
...
repo 目前是 **private**，公開前有一個 blocker：

- **Co-Intelligence 的翻譯字串目前 hardcode 在 `scripts/regenerate_bilingual.py` 跟 `scripts/assemble.py` 裡**。在結構保真重構（Round 2 + Round 3 的自動修復循環）過程中，為了讓重生 audit 一次過，assembler 被塞了 Co-Intelligence 致謝頁、版權頁、獻辭頁的 hardcoded 英文 → 繁中段落對。那些字串是 Ethan Mollick *Co-Intelligence: Living and Working with AI*（Portfolio/Penguin, 2024）的版權內容，repo 公開前必須抽出 skill 原始碼，移到書本獨立的譯文 table（放在 repo 外、run directory 底下）。
- `assemble.py` 裡的 `nav.xhtml` template 也寫死了 Co-Intelligence 的章節 label，需要改成從輸入 manifest 讀章節標題的通用 template。
- `scripts/regenerate_bilingual.py` 寫死 3 個 `/Users/fredchu/...` 絕對路徑，要改成必填 CLI args。

這些都不影響 skill 本身的設計——是 Codex 在自動重構時為了便宜地通過 audit 留下的痕跡。修法就是把那三支 script 重構乾淨，pipeline 其他部分本來就是 book-agnostic。

抽完之後，公開 release checklist：

- [ ] 把 Co-Intelligence-specific 譯文搬到外部譯文 table
- [ ] `regenerate_bilingual.py` 路徑改成 CLI args
- [ ] 加 `LICENSE`（MIT 或 Apache 2.0）
- [ ] 加 `CHANGELOG.md`
- [ ] 加 CI（GitHub Actions pytest）
- [ ] 設 GitHub topics：`claude-code`、`epub`、`translation`、`bilingual`、`traditional-chinese`
- [ ] repo visibility 切 public
