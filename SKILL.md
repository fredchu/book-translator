...
- **Cloud vLLM（Vast.ai／RunPod，2026-09-04 起）** — 沒有 Apple Silicon、或想比本機快 3–5 倍時，
  ...
  > 「跑飛餘裕」門檻把 fp8 壓到只剩併發 8。**第二版的門檻本身是錯的**——它算門檻用的
  > 120 秒逾時，翻譯器實際從來沒用過（翻譯器真正的逾時預設是 1800 秒，兩者差 15 倍，
  > 三席都以為那個 120 是真的，直到有 worker 動手接線才發現對不上）。改成不靠這個
  > 不存在的逾時之後，fp8 拿得到併發 16，貴的比例退回接近第一版，但**理由完全不同**——
  > 不是「原本猜對了」，是「原本的推理鏈跟原本的答案剛好都錯在同一個方向」。
  > 細節見 CHANGELOG「Corrected — the v1.1.0 concurrency probe's absolute threshold
  > never matched reality」那條。
  >
  > 同族教訓：2026-06-25 曾用自動化語域評分把 35B 升成預設，2026-08-09 讀者讀完一章推翻。
  > 這是同一族錯誤的第三次：**自動化的數字說不出讀者讀到的東西，這次連自動化的數字
  > 自己內部都對不上**。
  ...
  **併發（2026-09-10 起，雲端預設；判準 2026-09-10 spec-05 起換過一次）**：本機 omlx
  單一 GPU 維持循序。雲端 vLLM／SGLang 由 `cloud_llm.sh` 預設帶
  `--concurrent-chapters --max-concurrent-requests N`；直接跑 driver 時要同時加這兩個旗標，
  前者開跨章排程、後者控制全書總在途 request 上限。server 以最高候選 **32** 開機，
  client 則由自適應探針從最小的候選往上量，選到不再划算為止；不會膨脹成
  「章數 × 塊數」的無界 fan-out。

  舊表（16／24／32 的「120 秒餘裕」欄）**整欄作廢**——那個 120 秒逾時翻譯器從來沒真的
  用過，見 CHANGELOG「Corrected — the v1.1.0 concurrency probe's absolute threshold never
  ...
  **現在的判準是兩個相對指標，探針從 8 開始往上量，不設絕對門檻**：
  ...
  - **逾時不再是探針的輸入，是探針的輸出**：`derive_timeout()` 用「選中那一級」自己量到的
    ...
    不是兩個地方各自維護同一組數字（那正是這次要修的洞）。
  - **預設 16 只在探針整個跑不起來時當退路**，理由改成「唯一量過的卡上、品質訊號全 0 的
    最小值，保守起見沒量過的卡也預設用它」，不再跟任何百分比餘裕掛勾——理由變了，
    數字沒有跟著拉高。
