...
### Features
- 新增 `scripts/sglang_boot_health_check.py`：純唯讀的開機判讀，判六條並回三態
  （PASS=0／FAIL=1／UNKNOWN=3）。UNKNOWN 刻意避開 2，因為 argparse 用法錯誤原生
  就退 2——盯著計費機器做砍機決定時，把「指令打錯」讀成「還在開機」會直接多燒錢。
  缺行一律判 UNKNOWN，絕不判過。
- 新增 `scripts/redact_run_dir.py`：把 run 目錄裡的憑證塗掉，供「複製進 git 倉庫之前」用。

### Performance
- **探針量法整個換掉：閉環中段窗口。** 舊的用「該波總 token ÷ 該波牆鐘」算增幅，
  而牆鐘由該波**最長那一筆**決定——併發越高越容易抽到長塊、越容易被誤判成飽和。
  真機實測 N=24 實際 +26.6%，舊量法只算出 **+3.4%** → 早停選 16，**藏掉 53% 效能**。
  新做法每級 N 槽持續補送 T 秒，一筆完成立刻補一筆，只算中段窗口。
- **門檻改成不對稱**：早停一次藏 53%、多爬一級只多一分鐘，代價不對稱門檻就不該對稱。
  安全閘過就繼續爬，連兩級掉超過 5% 才停，**選最好那級不是最高那級**。
- **邊際效率門檻**：新最好要 > 最好 × (1 + 0.2 × (N/N_best − 1))。
  沒有這道，真機第一次跑新規則就選到**排隊高原** N=48（伺服器上限 32、多的在排隊），
  換 3.5% 吞吐卻付出中位延遲 +40%。門檻 0.20 從真機六級效率推出
  （真級最低 0.37、高原 0.07，中間空一大段）。
- **KV 夾**：候選 N ≤ `max_total_num_tokens` ÷ 3700（真機 129,742 → 35）。
  這條是算術事實不是統計判斷，零成本，從就緒摘要讀。

### Fixes
- **判活要檢查回應的形狀，不是只檢查有沒有回應。** 真機一趟探針零波成功卻判「伺服器活著」，
  真因是漏帶金鑰、全部 401——**有回應，但那個回應是驗證失敗不是伺服器狀態**。
  改成 `/get_server_info` 要含 `model_path`、`/metrics` 要解析得出真樣本。
- **探針候選夾實效上限**：從就緒摘要的 `max_running_requests` 讀，不從 `/get_server_info` 讀
  （後者回啟動參數，害我們三趟把「9 併發加排隊」量成「32 併發」）。
- **retract 偵測三層收緊**：舊的寬鬆比對抓到 Unix 時間戳（差值正好等於該波秒數）
  誤判成 retract、錯誤停在 N=8。改成先比對確切計數器名，找不到才退回嚴格 Prometheus
  解析並排除時間戳類計量；完全抓不到時回「不知道」而非 0。
- **每級加一發不計分的暖機請求**：第一波沒有前一波暖機，冷啟動或系統忙時最前面
  一兩筆特別慢就誤觸落後者門檻。真機 N=8 曾是 1.65 倍、門檻 2 倍——**那趟只是運氣好**。
- **中段窗口補守衛測試**：原本把排除比例改成 0 測試照樣全綠，等於核心可以被拿掉沒人發現。
- **候選改旗標覆蓋**，不再為了測試臨時改常數（那個臨時值曾污染另一席的測試結論）。
- **落盤前塗掉憑證**：`vast-instance-record.json` 的 `--api-key` 與 `jupyter_token`
  曾隨快照進 git。憑證是一次性的、機器已砍，但證據檔本來就不該帶。
- 三個「判準寫了但資料沒收」的欄位補上落盤：`max_in_flight`（事件計數非取樣）、
  `window_completions`、`distinct_prompt_hashes`。

### Documentation
- **fp8 成本第四次更正：貴約 18% 不是 83%。** 前三次都用探針選到的 N 算，
  而探針本身在低估。用三級持續負載證實的 N=32 重算，fp8 每本 0.153 對 int4 0.130。
- SKILL.md 的探針判準段整段改寫成 v1.2.0 的行為。
- 明文紀律：**探針的值只用來排序與選 N，成本與配方數字一律取自 10×N 持續負載**
  （探針對持續負載系統性高估約一成）。
